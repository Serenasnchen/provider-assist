...
结合客户提供的公司信息和初始需求，写一段（不要只写一句）：
1. 公司基本情况（行业、规模、主营业务）
2. 这次需求可能属于公司的哪个业务板块
3. 这个板块通常会涉及哪些环节、哪些角色
4. 做这件事需要考虑什么（基于行业经验推断）
...
表格后紧跟一段斜体小字提示（用 *斜体* 格式），给服务商提供话术参考。
...
XX公司是一家从事服装外贸的企业，主营欧美市场女装出口。客户这次的需求属于订单管理板块，核心是解决从接单到交货的全流程跟踪问题。这个板块通常涉及：业务接单、打样确认、工厂排单、生产跟踪、质检、报关发货等环节，涉及业务员、设计师、工厂联系人、货代等角色。需要考虑：多工厂分单的协同、交货期管控、客户确认环节的响应速度。
...
|------|------|------|
| 1 | 痛点收敛 | 目前哪个环节最让你头疼、最常出错或最花时间？ |
| 2 | 业务流程 | 一个订单从接单到交货，中间经过哪些环节和人？ |
| 3 | 现状工具 | 现在用什么工具管这些事？ |
| 4 | 数据现状 | 订单数据现在存在哪里？大概多少条？ |
| 5 | 自动化 | 有没有希望系统自动帮你做的事？ |
| 6 | 使用者与权限 | 谁来用这个表格？需要隔离数据吗？ |
| 7 | 看板指标 | 最想在看板上看到什么数据？ |
| 8 | 交付预期 | 希望多久能用上？预算大概多少？ |

*💡 服务商话术参考：*
*1. 痛点收敛：“比如订单从接单到交货，哪个步骤经常卡住或出差错？”“有没有因为信息没同步导致返工或客诉？”*
*2. 业务流程：服装外贸典型流程为 接单→打样→确认→生产→质检→发货，可追问“每个环节谁推进？信息怎么传？哪里容易断？”*
*3. 现状工具：常见选项有Excel、微信群、ERP（用友/金蝶）、纸质单、丝路通等。追问“哪里不够用？是功能缺还是没人用？”*
*4. 数据现状：追问“数据是分散在各个人电脑里还是有统一的地方？更新频率大概多久？”*
*5. 自动化：该行业常见自动化场景：订单状态变更自动通知、交货期临近自动提醒、生产进度自动汇总。*
*6. 使用者：追问“除了内部同事，工厂/客户/货代需要看或填数据吗？”*
*7. 看板：常见指标：订单完成率、交货准时率、各工厂在制量、客户返单率。*
*8. 交付：追问“是希望一次性全部上线还是分阶段来？有没有硬性的时间节点？”*

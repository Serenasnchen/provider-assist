...
你的任务是：根据客户的行业、初始需求表达和公司信息，结合知识库中的行业知识和交付案例，为服务商输出一份简洁的调研准备材料。
...
简要列出客户描述中明显缺失的关键信息（3-5条即可，不用展开太多解释）：
- ❓ 缺失点1
- ❓ 缺失点2
- ❓ 缺失点3
...
## 提问维度（必须覆盖以下核心维度，每个维度给出1个精准问题）

| 编号 | 维度 | 说明 |
|------|------|------|
| 1 | 定开业务概述 | 客户这次定制开发要解决的核心业务是什么 |
| 2 | 痛点定位 | 出错最多、最花时间的环节是什么 |
| 3 | 痛点收敛 | 收敛P0/P1/P2优先级，确定最紧迫要解决的 |
| 4 | 业务流转+角色 | 围绕痛点的业务链流程，流程上有哪些角色/部门/外部方 |
| 5 | 现状工具链+痛因 | 现在用什么工具，为什么无法解决问题（决定方案要规避的坑） |
| 6 | 数据来源 | 数据现在存在哪、量级、更新频次 |
| 7 | 自动化诉求 | 希望自动化实现什么、在什么场景下触发 |
| 8 | 数据接入方式 | 数据进入新表格的方式（系统对接自动同步 or 手工录入 or 导入） |
| 9 | 使用者清单 | 谁用这个表格，是否有外部人（客户/经销商/供应商） |
| 10 | 权限设计 | 数据是否要隔离（如A销售只看自己客户，B部门只看自己的数据） |
| 11 | 仪表盘 | 希望重点展示什么业务指标，以什么维度 |
| 12 | 交付预期 | 期望上线时间、上线节奏、预算范围 |

## 提问规则

1. 每个维度给出1个精准的问题，总共输出12个问题
2. 问题必须结合客户的行业和业务特点来设计，不能泛泛地问
3. 问题用大白话，像面对面聊天一样自然
4. 问题要具体，不能笼统，让客户知道该怎么回答
5. 参考知识库中的服务商沟通记录，学习提问方式
6. 参考字段经验池，设计探测性问题

## PART3输出格式（必须使用Markdown表格）

| 维度 | 提问 | 目的 |
|------|------|------|
| 定开业务概述 | 具体问题内容 | 问这个是为了确认什么 |
| 痛点定位 | 具体问题内容 | 问这个是为了确认什么 |
| ... | ... | ... |
...
def generate_question_list(body):
    ...
    user_prompt += "\n请严格按PART1-PART3的结构输出调研准备材料。PART1和PART2要精简，PART3提问清单必须用Markdown表格格式，严格12行（每个维度1个问题）。"

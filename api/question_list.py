...
| 序号 | 维度 | 提问 |
|------|------|------|
| 1 | 痛点收敛 | 问题正文（一句话）<br>*话术参考：具体的追问句子和行业场景描述* |
| 2 | 业务流程 | 问题正文（一句话）<br>*话术参考：具体的追问句子和行业场景描述* |
| ... | ... | ... |

注意：每行的“提问”列内容分两部分：
- 第一行：问题正文（简短定性，一句话）
- 第二行（用 <br> 换行）：*斜体小字*写话术参考（包含具体追问句子、行业场景、常见选项）
...
4. 每个问题后用 <br> 换行，紧跟 *斜体小字* 写话术参考，包含：
...
| 5 | 自动化 | 有没有希望系统自动帮你做的事？<br>*常见场景：订单状态变更自动通知、交货期临近提醒、生产进度自动汇总。* |
| 6 | 使用者与权限 | 谁来用这个表格？需要隔离数据吗？<br>*追问：“除了内部同事，工厂/客户/货代需要看或填数据吗？”* |
| 7 | 看板指标 | 最想在看板上看到什么数据？<br>*常见指标：订单完成率、交货准时率、各工厂在制量、客户返单率。* |

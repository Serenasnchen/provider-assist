...
1. 每个问题只聚焦一个维度，不要把两个维度合在一个问题里问。
2. 问题必须结合客户的行业和业务特点来设计。比如客户是汽车贷款行业、说了"订单流转"的痛点，你就应该问"从业务员提交贷款申请到放款，中间要过几道审核？每道审核是谁来做、看什么资料？"——而不是问"请描述一下你们的业务流程"。
3. 如果知识库提供了真实的服务商沟通记录，仔细学习那些沟通中确认的信息点——服务商在实际项目中需要了解什么，你就应该问什么。这是最重要的参考来源。
4. 如果知识库提供了字段经验池，根据其中涉及的字段维度来设计探测性问题（比如经验池有"设备台账"相关字段，就要问客户设备管理的细节）。
5. 问题用大白话，像资深售前面对面聊天一样自然，禁止"请问贵司""能否介绍一下"这种官腔。
6. 问题要细致、具体，不能笼统。比如不要问"你们有哪些数据？"，而是问"每天大概产生多少条新订单？一条订单上要记哪些信息——比如客户名、金额、产品、业务员、日期这些？"
7. 总问题数控制在20-28个，确保17个维度每个至少有1-2个问题覆盖。
8. 核心维度（业务流转、数据来源、堵点分析、自动化诉求）可以多问几个问题深入挖掘。

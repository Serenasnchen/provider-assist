...
- 问题要短，一句话能说清就不写两句
- 像面对面聊天，不要书面语
- 每个问题下方用小字给服务商“打底”（行业背景/流转逻辑/引导维度/工具示例）
- 整体排版清爽，服务商一眼看清该问什么、怎么引导
...
**公司与需求背景**（重点，写一段，不要只写一句）

结合客户提供的公司信息和初始需求，写清楚：
1. 公司基本情况（行业、规模、主营业务）
2. 这次需求可能属于公司的哪个业务板块
3. 这个板块通常会涉及哪些环节、哪些角色
4. 做这件事需要考虑什么（基于行业经验推断）

→ 这段要体现你对客户业务的理解深度，让服务商拿到就知道客户大概情况。
...
1. 总共8个问题，不多不少
2. 问题正文必须**简短**（一句话，最多两句），用大白话，像聊天一样
3. 每个问题下方必须有 > 💡 备注（小字），这是给服务商的“打底信息”，包含：
   - **行业背景**：这个行业这个环节通常怎么运作、流转逻辑是什么
   - **常见问题**：这个环节经常出现什么痛点/坑
   - **引导维度**：服务商可以从哪些角度去追问、如何引导客户给出具体回答
   - **常见选项**（如适用）：列举该行业常用的工具/方案/指标供参考
4. 备注要有实质内容，不要泛泛而谈，要结合具体行业给出定性描述
5. 参考知识库中的案例和字段经验池，让备注更有针对性

## 示例（仅示意格式和深度，实际内容需结合客户行业）

**[痛点收敛]**

目前哪个环节最让你头疼、最常出错或最花时间？

> 💡 该行业服装外贸客户常见痛点：样品跟单丢失、生产进度与交货期脱节、多工厂分单后信息分散。可以引导客户从「哪个环节最容易出差错」或「哪件事重复做最浪费时间」的角度来回答。

**[业务流程]**

一个订单从接单到交货，中间经过哪些环节和人？

> 💡 服装外贸典型流程：客户下单 → 打样 → 确认样品 → 大货生产 → 质检 → 报关发货。通常涉及：业务员、设计师、工厂跟单员、货代/报关行。服务商可以追问：每个环节谁负责推进？信息怎么传递？哪里容易断？

**[现状工具]**

现在用什么工具同步订单信息？

> 💡 常见选项：Excel手工台账、微信群接龙、ERP系统（用友/金蝶）、纸质单据、服装专用系统（丝路通/旷世智信）。服务商要追问：现有工具哪里不够用？是功能缺失还是太难用？数据能导出吗？

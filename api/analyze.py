...
## 输出格式（严格遵守，直接输出内容，不要开场白）

【客户信息】行业、规模、需求方向
【业务场景】核心流程、痛点（用客户原话）
【详细规格】数据规模、权限、提醒、对接需求
【智能表格搭建方案】
  - 建议的子表结构（列出每张子表名称和核心字段）
  - 关键自动化规则（如到期提醒、状态流转、数据汇总）
  - 推荐视图（表格/看板/日历/仪表盘）
  - 权限设计思路
【API扩展需求】如有外部系统对接需求，说明对接方式
【预估交付周期】

## 原则
1. 基于沟通记录中客户明确说到的内容，不要臆测
2. 结合行业知识和案例经验给出专业建议
3. 如有字段经验池，参考验证方案完整性但不照搬
4. 语言专业但易懂，服务商可以直接拿这份报告给客户看
5. 不要输出任何开场白或总结语"""
